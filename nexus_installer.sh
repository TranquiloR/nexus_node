...
install_dependencies() {
    ...
    sudo DEBIAN_FRONTEND=noninteractive apt install -y curl git screen build-essential pkg-config libssl-dev psmisc unzip
    ...
    # Установка protoc
    echo "Установка последней версии protoc..."
    if ! command -v protoc &> /dev/null || ! protoc --version | grep -q "25.0"; then
        echo "protoc либо не установлен, либо версия устарела. Устанавливаем версию 25.0..."
        wget https://github.com/protocolbuffers/protobuf/releases/download/v25.0/protoc-25.0-linux-x86_64.zip
        check_status "Не удалось скачать protoc"
        sudo unzip protoc-25.0-linux-x86_64.zip -d /usr/local
        check_status "Не удалось распаковать protoc"
        sudo chmod +x /usr/local/bin/protoc
        # Удаляем временный файл
        rm -f protoc-25.0-linux-x86_64.zip
        # Обновляем PATH в текущей сессии
        export PATH=$PATH:/usr/local/bin
    else
        echo "protoc версии 25.0 уже установлен."
    fi
